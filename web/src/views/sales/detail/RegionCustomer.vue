<template>
  <div class="region-customer">
    ...
    <div class="chart-section">
      <v-chart ref="chartRef" :option="chartOption" style="height: 340px" autoresize />
    </div>
...
async function loadData() {
  ...
  try {
    ...
    console.error('加载区域×客户出货排名失败:', error)
    tableData.value = []
  } finally {
    loading.value = false
    setLoading(false)
...
function buildChart() {
  if (tableData.value.length > 0) {
    const sortedData = [...tableData.value].sort((a, b) => b.salesAmount - a.salesAmount)
