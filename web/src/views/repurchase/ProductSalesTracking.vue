<template>
  <div class="product-sales-tracking">
    <!-- 筛选器 -->
    ...
    <div class="section-card">
      <div class="section-title">上市后销量表</div>
      <el-table :data="lifecycleData" stripe border size="small" max-height="450">
        ...
        <el-table-column prop="first_sale_date" label="首销日期" width="110" align="center" />
        <el-table-column prop="current_stock" label="在库量" width="80" align="right" />
        <el-table-column
          v-for="col in lifecycleMonthColumns"
          :key="col.prop"
          :prop="col.prop"
          :label="col.label"
          width="80"
...
import { getProductLifecycle, getProductCustomerCohort } from '@/api/repurchase'
...
const lifecycleMonthColumns = ref([])
...
async function loadData() {
  try {
    ...
    const data = res.data?.data || res.data || {}
    lifecycleData.value = data.lifecycle_data || []
    if (lifecycleData.value.length > 0) {
      const allKeys = Object.keys(lifecycleData.value[0])
      lifecycleMonthColumns.value = allKeys
        .filter(k => k.startsWith('+') || k === '首月')
        .sort((a, b) => {
          if (a === '首月') return -1
          if (b === '首月') return 1
      ...
      lifecycleMonthColumns.value = []
    }
  } catch (e) {
    console.error('加载商品生命周期表失败:', e)
    lifecycleData.value = []
  } finally {
    setLoading(false)
  }
